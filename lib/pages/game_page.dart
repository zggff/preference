...
class _GamePageState extends State<GamePage> {
  ...
  void resetGame() {
    displayGamesByPlayer = Map.fromEntries(
      s.players.keys.map((name) => MapEntry(name, true)),
    );
    displayGamesByDealer = Map.fromEntries(
      s.players.keys.map((name) => MapEntry(name, true)),
    );
    displayGamesByType = Map.fromEntries(
      GameType.values.map((t) => MapEntry(t, true)),
    );
  }
  ...
  bool _keyboardCallback(KeyEvent e) {
    ...
    } else if (e.logicalKey == LogicalKeyboardKey.keyR) {
      _rotate();
    }
    return false;
  }
  ...
  Future<void> displayPlayerInfo(BuildContext context, String name) async {
    await showDialog(
      context: context,
      builder: (_) => PlayerInfoDialog(gameState: s, name: name),
    );
  }
    ...
    return Column(
      ...
      children: [
          ...
          children: [
            ...
            Expanded(
              ...
              child: Text(
                ...
                style: TextStyle(
                  fontWeight:
                      game.type == GameType.raspas || !game.dark[game.player]!
                      ? FontWeight.normal
                      : FontWeight.bold,
        ...
        () {
          ...
          if (game.type == GameType.misere || game.type == GameType.neg) {
            taken.removeWhere((k, _) => k != game.player);
          } else if (game.type != GameType.raspas && game.taken.length == 4) {
          ...
          return Table(
            ...
            children: [
              TableRow(
                children: [
                  for (var name in taken.keys) Center(child: Text(name)),
                ],
              ),
              TableRow(
                children: [
                  for (var val in taken.values)
                    Center(child: Text("{}".format(val))),
                ],
              ),
            ],
          );
        ...
        if (game.bonus != null)
          Text(
            "{}".format(
              game.bonus! == Bonus.bird ? "на птичке" : "на бомбочке",
            ),
          ),
      ],
    );
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Padding(
                  ...
                  child: _buildGameListChild(game),
                ),
              ),
            ),
      ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                  ...
                  TableRow(
                    children: [
                      TableCell(
                        ...
                        child: TextButton(
                          onPressed: () {
                            var sel = displayGamesByDealer.values.any((k) => k);
                            displayGamesByDealer.updateAll((k, v) => !sel);
                            setState(() {});
                          },
                  ...
                  TableRow(
                    children: [
                      TableCell(
                        ...
                        child: TextButton(
                          onPressed: () {
                            var sel = displayGamesByPlayer.values.any((k) => k);
                            displayGamesByPlayer.updateAll((k, v) => !sel);
                            setState(() {});
                          },
                  ...
                  TableRow(
                    children: [
                      TableCell(
                        child: TextButton(
                          onPressed: () {
                            var sel = displayGamesByType.values.any((k) => k);
                            displayGamesByType.updateAll((k, v) => !sel);
                            setState(() {});
                          },
                      ...
                      for (var k in displayGamesByType.keys)
                        TableCell(
                          child: Checkbox(
                            ...
                            onChanged: (change) {
                              displayGamesByType[k] = !displayGamesByType[k]!;
                              setState(() {});
                            },
                          ),
                        ),
            ...
            Expanded(flex: 14, child: _buildGameList()),
          ],
        ),
      ),
    );
      ...
      children: [
        Expanded(
          ...
          child: GestureDetector(
            onVerticalDragStart: (det) => _dragStart = det.globalPosition.dy,
